...
In this example of a simulation, we consulted \cite{daniel:gilles}. The simulation runs by default 100000 iterations. At each iterations, an action is selected based on predefined probabilities. 50% cancel orders, 20% market orders and 30% limit orders. When a limit order is placed, the price of the new order can be either inside or outside the current spread. This probability is chosen uniformly. The source code of the simulation is shipped with the package.
