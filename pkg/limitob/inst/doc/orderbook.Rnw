...
Finally, users can easily plot the trade data by using \texttt{plot.trade}.
...
In this example of a simulation, we consulted \cite{daniel:gilles}. The simulation runs by default 1000 iterations. At each iterations, an action is selected based on predefined probabilities. 50\% cancel orders, 20\% market orders and 30\% limit orders. When a limit order is placed, the price of the new order can be either inside or outside the current spread. This probability is chosen uniformly. The source code of the simulation is shipped with the package.
...
\section*{Future work}
The package limitob is at its very early stage and there are rooms for improvement. We are looking into extending the simulation into an trading exchange model. The exchange handles orders from multiple agents while at the same time sending the order feed to the orderbook object. Furtheremove, we would like to incorporate the ability to backtest trading strategies into the limitob package.
\section*{Conclusion}
In this short article, we have described the limitob package. limitob aims to provide user-friendly statistical and visualization tools to analyze limit orders data. We demonstrated the functionalities of the package through a series of examples. Users who deal frequently with limit orders data (i.e high frequency strategies and traders) will find the package useful. While the curent package is not complex enough to be a stand-alone platform for developing trading stategies, it can be a good start for testing and hypothesizing new trading conjecture.
